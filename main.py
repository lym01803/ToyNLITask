...
if __name__ == '__main__':
    ...
    train_loader = DataLoader(nluDataset(trainset), batch_size=batchsize, shuffle=True)
    ...
    linear_probe = LinearProbeLayer().to(device)
    ...
    top_5_steps = []
    with tqdm(initial=iter_step, total=max_iters, disable=False) as tqdmbar:
        while iter_step < max_iters:
            if len(data_pieces_to_fetch) > 0:
                data_piece = data_pieces_to_fetch.pop()
            else:
                data_piece = next(train_loader)
            batch, lengths, labels = prepare_for_batch(data_piece)
            if max(lengths) * len(lengths) > max_batch_len:
                split1, split2 = half_data_piece(data_piece)
                data_pieces_to_fetch.append(split2)
                data_pieces_to_fetch.append(split1)
                continue
            # print(len(lengths), max(lengths), len(data_pieces_to_fetch))
            iter_step += 1
            ...
            loss_record = loss_record[-record_interval:]
            with torch.no_grad():
                predict = torch.argmax(output, dim=1)
                acc = torch.sum(predict == labels) / labels.shape[0]
                acc_record.append(acc.item())
                acc_record = acc_record[-record_interval:]
            tqdmbar.set_description(f'step/total: {iter_step}/{max_iters}, recent_loss: {sum(loss_record) / len(loss_record) :.6f}, recent_acc: {sum(acc_record) / len(acc_record) :.6f}, lr: {scheduler.get_last_lr()}')
            tqdmbar.update(1)

            if iter_step % save_interval == 0 or iter_step == 1:
                torch.save(
                    dict(model=linear_probe.state_dict()),
                    save_name + "-last.pth"
                )
                torch.save(
                    dict(model=bertmodel.state_dict()),
                    save_name2 + "-last.pth"
                )
            avg_train_acc = sum(acc_record) / len(acc_record)

            if iter_step % valid_interval == 0 or iter_step == 1:
                valid_acc = []
                valid_loss = []
                ...
                acc_score = 0.2 * avg_train_acc + 0.8 * avg_valid_acc
                if len(top_5_steps) < 5 or top_5_steps[-1][-1] < acc_score :
                    torch.save(
                        dict(model=linear_probe.state_dict()),
                        save_name + f"-{iter_step}.pth"
                    )
                    torch.save(
                        dict(model=bertmodel.state_dict()),
                        save_name2 + f"-{iter_step}.pth"
                    )
                    top_5_steps.append((iter_step, acc_score))
                    top_5_steps.sort(key=lambda x : -x[-1])
                    if len(top_5_steps) > 5:
                        pop_item = top_5_steps.pop()
                        file1 = save_name + f"-{pop_item[0]}.pth"
                        file2 = save_name2 + f"-{pop_item[0]}.pth"
                        if os.path.exists(file1):
                            try:
                                os.system(f"rm -f {file1}")
                            except:
                                pass 
                        if os.path.exists(file2):
                            try:
                                os.system(f"rm -f {file2}")
                            except:
                                pass 
                    print(top_5_steps)
